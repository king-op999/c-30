// api/index.js - BRONX OSINT ADVANCED V2.0
...
const DATA_FILE = path.join(DATA_DIR, 'bronx_adv_data.json');
const UPI_ID = process.env.UPI_ID || '8509561376@ibl';
const UPI_NAME = process.env.UPI_NAME || 'BRONX_ULTRA';
const PROFILE_PIC = process.env.PROFILE_PIC || 'https://i.ibb.co/sJwBtnWd/IMG-20260413-080502-825.jpg';
...
// ========== ALL API ENDPOINTS ==========
const SERVICES = {
    number: { name: '📱 Number Info', credit: 5, icon: '📱', color: '#0096ff', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/number', param: 'num', key: 'op', desc: 'Basic mobile number lookup with carrier info' },
    aadhar: { name: '🆔 Aadhar Info', credit: 10, icon: '🆔', color: '#00c853', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/aadhar', param: 'num', key: 'op', desc: 'Aadhaar card details lookup' },
    vehicle: { name: '🚗 Vehicle Info', credit: 10, icon: '🚗', color: '#ff6d00', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/vehicle', param: 'vehicle', key: 'op', desc: 'Vehicle registration & owner details' },
    ff: { name: '🎮 Free Fire', credit: 3, icon: '🎮', color: '#ff1744', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/ff', param: 'uid', key: 'op', desc: 'Free Fire player ID info' },
    email: { name: '📧 Email Lookup', credit: 5, icon: '📧', color: '#ff9100', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/leakinfo', param: 'term', key: 'op', desc: 'Email leak & breach lookup' },
    tg: { name: '📲 TG to Num', credit: 12, icon: '📲', color: '#00b8d4', api: 'https://tg-ifo-babu-0.vercel.app/tracex', param: 'username', key: 'BRONXop', desc: 'Telegram username to number info' },
    upi: { name: '💰 UPI Info', credit: 5, icon: '💰', color: '#7c4dff', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/upi', param: 'upi', key: 'op', desc: 'UPI ID account details' },
    ...
    numleak: { name: '🔓 Number Leak', credit: 8, icon: '🔓', color: '#ff5252', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/numleak', param: 'num', key: 'op', desc: 'Check if number was leaked' },
    ifsc: { name: '🏦 IFSC Info', credit: 3, icon: '🏦', color: '#2979ff', api: 'https://osint-bronx-ultra-2-0.onrender.com/api/key-bronx/ifsc', param: 'ifsc', key: 'op', desc: 'Bank IFSC code details' },
};
...
app.post('/api/register', (req, res) => {
    ...
    if (db.users[username]) return res.json({ error: 'Username taken' });
    ...
    saveDB();
    res.json({ success: true, token, message: '✅ Registered! 1 free credit.' });
});
...
app.post('/api/login', (req, res) => {
    ...
    res.json({ success: true, token, credit: user.credit });
});
...
app.get('/api/service/:name', (req, res) => {
    ...
    const token = req.query.token || req.headers['x-auth-token'];
    const user = db.users[token];
    if (!user) return res.json({ error: 'Login required' });
    if (user.banned) return res.json({ error: 'Account banned' });
    if (user.credit < service.credit) return res.json({ error: `Insufficient credits! Need ${service.credit}, have ${user.credit}` });
    const value = req.query[service.param] || req.query.q || req.query.query;
    if (!value) return res.json({ error: `Missing ${service.param} parameter` });
    ...
    axios.get(url, { timeout: 30000 })
        .then(resp => {
            ...
            delete data.credit; delete data.owner; delete data.by; delete data.channel; delete data.api_by; delete data.key_note;
            data.api_info = { service: service.name, credit_used: service.credit, remaining: user.credit };
            saveDB();
            res.json(data);
        })
        .catch(e => {
            ...
            if (db.users[user.username]) db.users[user.username].credit = user.credit;
            saveDB();
            res.json({ error: 'API error - credit refunded' });
        });
});
...
app.post('/api/create-payment', (req, res) => {
    ...
    saveDB();
    res.json({ success: true, payment_id: paymentId, upi_link: upiLink, qr: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(upiLink)}` });
});
...
app.post('/api/admin-login', (req, res) => {
    ...
    if (username === ADMIN_USER && password === ADMIN_PASS) {
        ...
        db.users[token] = { username: 'admin', credit: 999, role: 'admin', token, banned: false };
        saveDB();
...
function renderHome(user) {
    ...
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>BRONX OSINT ADVANCED</title>
...
nav{position:sticky;top:0;z-index:1000;background:rgba(2,0,16,.9);border-bottom:1px solid var(--brd);padding:10px 24px;display:flex;justify-content:space-between;align-items:center;backdrop-filter:blur(30px)}
...
.upi-box{background:rgba(0,0,0,.3);border:1px solid var(--brd);border-radius:14px;padding:16px;text-align:center;margin:12px 0}
.upi-box code{display:block;background:#000;color:var(--acc2);padding:10px;border-radius:8px;font-size:11px;margin:8px 0;word-break:break-all}
.qr-img{max-width:160px;margin:10px auto;display:block;border-radius:12px;border:2px solid rgba(255,180,0,.15)}
.pay-link{display:block;background:linear-gradient(135deg,#ffb400,#ff8c00);color:#000;padding:14px;border-radius:14px;font-weight:700;text-decoration:none;font-family:'Orbitron',sans-serif;letter-spacing:2px;margin-top:8px}
...
<div class="container">
    <div class="hero">
        ...
        <h1>BRONX OSINT ADVANCED</h1>
        <p class="sub">🔍 10+ Services · 🪙 Credit System · 💳 UPI Payments · 🇮🇳 Indian Time</p>
    </div>
...
<div class="modal" id="paymentModal"><div class="modal-content">
    ...
    <div class="upi-box">
        <p style="color:#666;font-size:10px">Scan QR or Click Pay Button</p>
        <img id="qrImage" class="qr-img" src="" alt="QR">
        <code id="upiId">UPI: ${UPI_ID}</code>
        <a id="upiLink" href="#" class="pay-link" target="_blank">⚡ PAY NOW</a>
    </div>
    <p style="color:#ff8c00;font-size:10px;text-align:center">After payment, DM @BRONX_ULTRA with Payment ID</p>
    <p style="color:#fff;font-size:13px;text-align:center;font-family:monospace" id="paymentId"></p>
    <button class="btn-close" onclick="closePayment()">✕ CLOSE</button>
</div></div>

<footer><p class="fb">BRONX OSINT ADVANCED V2.0</p></footer>
...
async function doSearch(){var v=document.getElementById('searchInput').value.trim();if(!v)return;var rb=document.getElementById('resultBox');rb.style.display='block';rb.style.color='#00d4ff';rb.textContent='🔍 Searching...';try{var r=await fetch('/api/service/'+currentService+'?token='+TOKEN+'&q='+encodeURIComponent(v));var d=await r.json();rb.style.color=d.error?'#ff4444':'#00ff88';rb.textContent=JSON.stringify(d,null,2);if(!d.error)setTimeout(()=>location.reload(),1500)}catch(e){rb.style.color='#ff4444';rb.textContent='Error: '+e.message}}
function buyCredit(cr,pr){if(!TOKEN){location.href='/login';return}fetch('/api/create-payment?token='+TOKEN,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan_credit:cr,plan_price:pr})}).then(r=>r.json()).then(d=>{if(d.success){document.getElementById('upiId').textContent='UPI: ${UPI_ID}';document.getElementById('upiLink').href=d.upi_link;document.getElementById('qrImage').src=d.qr;document.getElementById('paymentId').innerHTML='📋 ID: <b style="color:#ffb400">'+d.payment_id+'</b>';document.getElementById('paymentModal').classList.add('show')}})}
...
.card button{width:100%;padding:15px;background:linear-gradient(135deg,#0096ff,#00d4ff);color:#fff;border:none;border-radius:14px;font-weight:700;cursor:pointer;font-family:'Orbitron',sans-serif;letter-spacing:3px;font-size:14px}
.card a{color:#0096ff;font-size:11px;text-decoration:none}.msg{color:#ff4444;font-size:11px;margin-top:8px;display:none}
</style></head><body><div class="card"><h2>🔐 LOGIN</h2><p class="sub">BRONX OSINT</p><input type="text" id="u" placeholder="Username"><input type="password" id="p" placeholder="Password"><button onclick="login()">AUTHENTICATE</button><p class="msg" id="msg"></p><p style="margin-top:14px;color:#444;font-size:11px"><a href="/register">Create Account</a> | <a href="/">Home</a></p></div><script>
async function login(){var u=document.getElementById('u').value,p=document.getElementById('p').value,m=document.getElementById('msg');if(!u||!p){m.style.display='block';m.textContent='Fill all fields';return}try{var r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});var d=await r.json();if(d.success){localStorage.setItem('token',d.token);location.href='/?token='+d.token}else{m.style.display='block';m.textContent=d.error}}catch(e){m.textContent='Error'}}</script></body></html>`}
...
.card{background:rgba(8,8,30,.85);padding:45px 35px;border-radius:24px;width:400px;border:1px solid rgba(139,0,255,.08);text-align:center}
.card h2{color:#fff;font-family:'Orbitron',sans-serif;font-size:26px;letter-spacing:3px;margin-bottom:6px}
.card .sub{color:#444;font-size:11px;letter-spacing:3px;margin-bottom:24px}
.card input{width:100%;padding:15px;background:rgba(0,0,0,.5);border:1px solid rgba(255,255,255,.04);border-radius:14px;color:#fff;font-size:14px;outline:none;margin-bottom:12px;font-family:'Rajdhani',sans-serif}
.card button{width:100%;padding:15px;background:linear-gradient(135deg,#8b00ff,#0096ff);color:#fff;border:none;border-radius:14px;font-weight:700;cursor:pointer;font-family:'Orbitron',sans-serif;letter-spacing:3px}
.card a{color:#8b00ff;font-size:11px;text-decoration:none}.msg{color:#ff4444;font-size:11px;margin-top:8px;display:none}
...
async function register(){var u=document.getElementById('u').value,p=document.getElementById('p').value,m=document.getElementById('msg');try{var r=await fetch('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});var d=await r.json();if(d.success){localStorage.setItem('token',d.token);location.href='/?token='+d.token}else{m.style.display='block';m.textContent=d.error}}catch(e){}}
</script></body></html>`}
...
.card h2{color:#ffb400;font-family:'Orbitron',sans-serif;font-size:26px;margin-bottom:20px}
.card input{width:100%;padding:15px;background:rgba(0,0,0,.5);border:1px solid rgba(255,255,255,.04);border-radius:14px;color:#fff;font-size:14px;outline:none;margin-bottom:12px;font-family:'Rajdhani',sans-serif}
.card button{width:100%;padding:15px;background:linear-gradient(135deg,#ffb400,#ff8c00);color:#000;border:none;border-radius:14px;font-weight:700;cursor:pointer;font-family:'Orbitron',sans-serif}
.msg{color:#ff4444;font-size:11px;margin-top:8px;display:none}
</style></head><body><div class="card"><h2>👑 ADMIN</h2><input type="text" id="u" placeholder="Username"><input type="password" id="p" placeholder="Password"><button onclick="login()">LOGIN</button><p class="msg" id="msg"></p></div><script>
async function login(){var u=document.getElementById('u').value,p=document.getElementById('p').value;try{var r=await fetch('/api/admin-login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});var d=await r.json();if(d.success)location.href=d.redirect;else{document.getElementById('msg').style.display='block';document.getElementById('msg').textContent=d.error}}catch(e){}}
</script></body></html>`}
...
function renderAdmin(user){
    ...
    const payHTML=db.payments.slice(-20).reverse().map(p=>`<tr><td><code>${p.id}</code></td><td>@${p.username}</td><td>🪙 ${p.credit}</td><td>₹${p.amount}</td><td style="color:${p.status==='approved'?'#00ff88':'#ffb400'}">${p.status}</td><td>${new Date(p.timestamp).toLocaleString('en-IN')}</td><td>${p.ip}</td><td>${p.status==='pending'?`<button class="ab a-g" onclick="approvePayment('${p.id}','${p.username}',${p.credit})">✅ APPROVE</button>`:'✅'}</td></tr>`).join('');
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>ADMIN</title><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@300;400;600;700&display=swap" rel="stylesheet"><style>
...
.tabs{display:flex;gap:6px;margin-bottom:14px}.tab{padding:10px 18px;background:var(--sur);border:1px solid var(--brd);border-radius:10px;color:#665500;cursor:pointer;font-size:11px}.tab.on{background:rgba(255,180,0,.06);border-color:#ffb400;color:#fff}
...
input{padding:10px;background:rgba(0,0,0,.5);border:1px solid var(--brd);border-radius:10px;color:#fff;font-size:12px;outline:none;font-family:'Rajdhani',sans-serif;margin-right:8px}
...
<div class="panel on" id="panel-users"><div class="section"><h3>👥 USERS</h3><div style="max-height:400px;overflow:auto"><table><tr><th>USER</th><th>CREDIT</th><th>IP</th><th>STATUS</th><th>ACT</th></tr>${usersHTML}</table></div></div></div>
<div class="panel" id="panel-payments"><div class="section"><h3>💳 PAYMENT LOGS</h3><div style="max-height:400px;overflow:auto"><table><tr><th>ID</th><th>USER</th><th>CR</th><th>₹</th><th>STATUS</th><th>DATE</th><th>IP</th><th>ACT</th></tr>${payHTML}</table></div></div></div>
<div class="panel" id="panel-addcredit"><div class="section"><h3>🪙 ADD CREDIT</h3><input type="text" id="addUser" placeholder="Username"><input type="number" id="addCredit" placeholder="Amount"><button class="btn" onclick="addCredit()">ADD</button></div></div>
<div class="panel" id="panel-ips"><div class="section"><h3>🛡 BANNED IPs</h3>${db.bannedIPs.map(ip=>`<code>${ip}</code> <button class="ab a-g" onclick="unbanIP('${ip}')">UNBAN</button><br>`).join('')||'<p style="color:#444">None</p>'}<br><input type="text" id="banIp" placeholder="IP"><button class="btn" onclick="banIP()">BAN</button></div></div>
</div>
<script>
...
async function addCredit(){var u=document.getElementById('addUser').value,c=document.getElementById('addCredit').value;var r=await ac('/api/admin/add-credit',{username:u,credit:parseInt(c)});alert(r.message||r.error);location.reload()}
...
async function banIP(){var ip=document.getElementById('banIp').value;await ac('/api/admin/ban-ip',{ip:ip});location.reload()}
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 BRONX OSINT ADVANCED on ${PORT}`));
module.exports = app;
